<%@ Page Title="" Language="C#" MasterPageFile="~/SoftProg.Master" AutoEventWireup="true" CodeBehind="Personal.aspx.cs" Inherits="FrontTA.GestionAcademica.Personal" %>

<asp:Content ID="Content1" ContentPlaceHolderID="cph_Title" runat="server">
    Colegio Rafael Mariscal Quintanilla - Búsqueda de Personal
</asp:Content>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cph_Scripts" runat="server">
    <style>
        ...
        /* Encabezados con sorting, negros sin subrayado */
        .table-header a,
        .table-header a:link,
    ...
    <script>
        (function () {
            function getTable() { return document.getElementById('<%= gvPersonal.ClientID %>'); }
            ...
            function wireUp() {
                ...
                if (btnAdd) {
                    ...
                    btnAdd.addEventListener('click', function (e) {
                        e.preventDefault();
                        window.location.href = "<%= ResolveUrl("~/GestionAcademica/CrearPersonal.aspx") %>";
                    });
                }
                if (btnView) {
                    btnView.addEventListener('click', function () {
                        ...
                        window.location.href = "<%= ResolveUrl("~/GestionAcademica/ConsultarPersonal.aspx") %>?id=" + id;
                    });
                }
                if (btnEdit) {
                    btnEdit.addEventListener('click', function () {
                        const sel = getSelected(); if (!sel) return;
                        const id = sel.getAttribute('data-id');
                        window.location.href = "<%= ResolveUrl("~/GestionAcademica/EditarPersonal.aspx") %>?id=" + id;
                    });
                }
                ...
                if (btnDel && overlay) {
                    btnDel.addEventListener('click', function () {
                        ...
                        // Asignamos el ID de la fila seleccionada al HiddenField
                        const hiddenField = document.getElementById('<%= hfPersonalDelete.ClientID %>');
                        hiddenField.value = sel.getAttribute('data-id');
...
<asp:Content ID="Content3" ContentPlaceHolderID="cph_Contenido" runat="server">
    <div class="container-fluid familias-container">
        ...
        <div class="toolbar mb-2">
            <div class="icon-bar">
                <button type="button" id="btnAdd" class="btn btn-icon btn-add" title="Añadir personal">
                    <i class="fa-solid fa-user"></i>
                </button>
                <button type="button" id="btnView" class="btn btn-icon btn-disabled" title="Consultar" disabled>
                    <i class="fa-solid fa-eye"></i>
                </button>
                <button type="button" id="btnEdit" class="btn btn-icon btn-disabled" title="Editar" disabled>
                    <i class="fa-solid fa-pen"></i>
                </button>
                <button type="button" id="btnDelete" class="btn btn-icon btn-disabled" title="Eliminar" disabled>
                    <i class="fa-solid fa-trash"></i>
                </button>
            </div>
            <span id="subTitle" class="header-cta" >Gestión Personal</span>
        </div>
        ...
        <div class="busqueda-box mb-4">
            <div class="row g-3 align-items-center">
                ...
                <div class="col-md-3">
                    ...
                    <asp:TextBox ID="txtNombre" runat="server" CssClass="form-control" MaxLength="120" />
                </div>
                <div class="col-md-3">
                    <label class="form-label">Apellido Paterno:</label>
                    <asp:TextBox ID="txtApePat" runat="server" CssClass="form-control" MaxLength="120" />
                </div>
                <div class="col-md-3">
                    <label class="form-label">Apellido Materno:</label>
                    <asp:TextBox ID="txtApeMat" runat="server" CssClass="form-control" MaxLength="120" />
                </div>
            </div>
            <div class="row mt-2">
        ...
        <div class="tabla-box">
            <asp:GridView ID="gvPersonal" runat="server"
                ...
                <Columns>
                    <asp:BoundField DataField="personal_id" HeaderText="ID" SortExpression="Id">
                        <ItemStyle Width="110px" />
                    </asp:BoundField>
                    <asp:BoundField DataField="nombre" HeaderText="Nombre" SortExpression="Nombre" />
                    <asp:BoundField DataField="apellido_paterno" HeaderText="Apellido Paterno" SortExpression="ApellidoPaterno" />
                    <asp:BoundField DataField="apellido_materno" HeaderText="Apellido Materno" SortExpression="ApellidoMaterno" />
                    <asp:BoundField DataField="dni" HeaderText="DNI" SortExpression="Dni">
                        <ItemStyle Width="140px" />
                    </asp:BoundField>
                    <asp:BoundField DataField="correo_electronico" HeaderText="Email" SortExpression="Email" />
                    <asp:BoundField DataField="telefono" HeaderText="Num teléfono" SortExpression="Telefono">
                        <ItemStyle Width="160px" />
                    </asp:BoundField>
                </Columns>
                <EmptyDataTemplate>
                    <div class="empty-state">No se encontró personal con los criterios ingresados.</div>
                </EmptyDataTemplate>
            </asp:GridView>
        </div>
        ...
        <div id="confirmDelete" class="confirm-overlay" aria-hidden="true">
            <div class="confirm-box">
                <div class="confirm-title">¿Estás seguro que deseas eliminar al personal?</div>
 ...
 <i class="fa-solid fa-check"></i>
                    ...
                    <asp:HiddenField ID="hfPersonalDelete" runat="server" Value="" />
