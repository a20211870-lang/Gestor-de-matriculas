...
<asp:Content ID="Content2" ContentPlaceHolderID="cph_Scripts" runat="server">
    <style>
        ...
        .overlay .tabla-box {
            max-height: 300px;        /* Ajusta según el tamaño de tu modal */
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // helper para obtener elemento y logear si falta
        function get(id) {
        ...
        // client IDs server-side (asegúrate que los controles existan en code-behind)
        const gvAlumnoId = 'gvAlumnoModal';            // tienes ClientIDMode="Static"
        const gvAulaId = 'gvAulaModal';

        // usa los ClientID reales de los LinkButtons generados por ASP.NET
        ...
        function wireSelectableGrid(gridId, okBtnClientId) {
            ...
            if (!gv) { console.error('Grid not found:', gridId); return; }
            if (!ok) { console.error('OK button not found:', okBtnClientId, ' — ensure control exists and ClientID is correct'); return; }

            // Inicial: deshabilitar ok si es input/button, o añadir clase de "disabled" si no soporta disabled
            try { ok.disabled = true; } catch (e) { ok.classList.add('disabled-link'); ok.setAttribute('aria-disabled', 'true'); }

            // Delegate click to tbody (si existe) para manejar filas dinámicas
            const tbody = gv.querySelector('tbody') || gv;
            tbody.addEventListener('click', function (e) {
                const tr = e.target.closest('tr.data-row');
                if (!tr || !gv.contains(tr)) return; // click fuera de fila válida

                // quitar selección previa (solo sobre filas visibles con data-row)
                gv.querySelectorAll('tr.data-row').forEach(r => r.classList.remove('row-selected'));
                tr.classList.add('row-selected');

                // habilitar ok
                try { ok.disabled = false; ok.classList.remove('disabled-link'); ok.setAttribute('aria-disabled', 'false'); }
                catch (ex) { ok.classList.remove('disabled-link'); ok.setAttribute('aria-disabled', 'false'); }

                // small visual focus for accessibility
        ...
        console.info('wireSelectableGrid attached. Check these elements:');
        console.log('gvAlumno:', document.getElementById(gvAlumnoId)); // typo-safe print
        console.log('btnAluOk:', document.getElementById(btnAluOkClientId));
        console.log('gvAula:', document.getElementById(gvAulaId));
        console.log('btnAulaOk:', document.getElementById(btnAulaOkClientId));
    });
    ...
    function ConfirmAlumno() {
        ...
        // Llenar los controles ASP.NET
        document.getElementById("<%= txtAlumno.ClientID %>").value =
            `${apePat} ${apeMat}, ${nombre}`;
        document.getElementById("<%= hdnAlumnoId.ClientID %>").value = id;
    ...
    function ConfirmAula() {
        const tr = document.querySelector("#gvAulaModal tr.row-selected");
        if (!tr) return false;

        const id = tr.getAttribute("periodo_aula_id"); // <— este es el ID REAL
        const nombreAula = tr.cells[0].innerText.trim();
        const vacantes = tr.cells[2].innerText.trim();

        document.getElementById("<%= txtAula.ClientID %>").value = nombreAula;
        document.getElementById("<%= txtVacantes.ClientID %>").value = vacantes;
        document.getElementById("<%= txtCapacidad.ClientID %>").value = vacantes;

        document.getElementById("<%= hdnAulaId.ClientID %>").value = id;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const txtNomAula = document.getElementById("f_nomAula");
        const txtNomGrado = document.getElementById("f_nomGrado");
        const btnBuscarAula = document.getElementById("btnBuscarAula");
        const grid = document.getElementById("gvAulaModal");

        if (!txtNomAula || !txtNomGrado || !btnBuscarAula || !grid) {
            console.warn("Filtro Aula: elementos no encontrados.");
            return;
        }

        // Helper: obtiene todos los th del header (thead o primer tr del table)
        function getHeaderCells(table) {
            let ths = table.querySelectorAll('thead th');
            if (ths.length) return Array.from(ths);
            // fallback: primer tr que tenga th dentro (puede ser directamente en table)
            ...
            // si no hay th, intentar usar la primera fila (td) como "header" textual
            const firstRowTds = firstHeaderRow.querySelectorAll('td');
            return firstRowTds.length ? Array.from(firstRowTds) : [];
        }

        // Buscar índice de columna cuyo header incluya texto (case-insensitive)
        function findColIndexByHeader(table, headerText) {
            const headers = getHeaderCells(table);
            headerText = headerText.toLowerCase().trim();
            for (let i = 0; i < headers.length; i++) {
                const txt = (headers[i].innerText || headers[i].textContent || '').toLowerCase().trim();
                if (!txt) continue;
                if (txt === headerText || txt.indexOf(headerText) !== -1) return i;
            }
            return -1;
        }
        ...
        btnBuscarAula.addEventListener("click", function () {
            ...
            // recalculamos índices cada vez (por si ASP.NET cambió columnas)
            // Nota: si tu header real dice "Nombre de Aula" y "Nombre de Grado Académico",
            // pasa la porción distintiva: "Nombre de Aula" y "Nombre de Grado"
            const idxAula = findColIndexByHeader(grid, "Nombre de Aula");
            const idxGrado = findColIndexByHeader(grid, "Nombre de Grado");

            if (idxAula === -1 && idxGrado === -1) {
                console.warn("Filtro Aula: no se encontraron headers 'Nombre de Aula' ni 'Nombre de Grado'.");
                return;
            }

            // obtener filas de datos (asegúrate de usar la clase que aplicaste: data-row)
            const rows = grid.querySelectorAll("tr.data-row");

            rows.forEach(row => {
                const cells = row.querySelectorAll("td");
                // si el índice encontrado es mayor que celdas, se toma como vacío
                const textoAula = (idxAula >= 0 && cells[idxAula]) ? (cells[idxAula].innerText || '').toLowerCase() : "";
                const textoGrado = (idxGrado >= 0 && cells[idxGrado]) ? (cells[idxGrado].innerText || '').toLowerCase() : "";
                ...
                if (coincideAula && coincideGrado) row.style.display = "";
...
<div class="card-form">
    ...
    <div class="action-buttons mt-3">
        <asp:LinkButton ID="btnGuardar" runat="server" CssClass="btn-accept"
            OnClick="btnGuardar_Click" ToolTip="Guardar cambios y volver">
            ...
            <div class="modal-box">
                <div class="modal-inner">
                    ...
                    <div class="busqueda-box">
                        <div class="row g-3 align-items-end">
                            <div class="col-md-2"><label class="mini-label">Código Familia</label><asp:TextBox ID="txtCodFam_Alum" runat="server" CssClass="form-control"></asp:TextBox></div>
                            <div class="col-md-2"><label class="mini-label">Apellido Paterno</label><asp:TextBox ID="txtApePat_Alum" runat="server" CssClass="form-control"></asp:TextBox></div>
                            <div class="col-md-2"><label class="mini-label">Apellido Materno</label><asp:TextBox ID="txtApeMat_Alum" runat="server" CssClass="form-control"></asp:TextBox></div>
                            <div class="col-md-2"><label class="mini-label">DNI</label><asp:TextBox ID="txtdni_Alum" runat="server" CssClass="form-control"></asp:TextBox></div>
                            <div class="col-md-2"><label class="mini-label">Nombre</label><asp:TextBox ID="txtNombre_Alum" runat="server" CssClass="form-control"></asp:TextBox></div>
                ...
                <div class="modal-inner">
                    ...
                    <div class="tabla-box grid-selectable">
                        <asp:GridView ID="gvAulaModal" runat="server" AutoGenerateColumns="False"
                            ...
                            <HeaderStyle CssClass="table-header" />
                            <Columns>
                                <asp:BoundField DataField="vacantes_ocupadas" HeaderText="Capacidad" Visible="false" />

                                <asp:TemplateField HeaderText="Nombre de Aula">
                                    <ItemTemplate><%# Eval("aula.nombre") %></ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField HeaderText="Nombre de Grado">
                                    <ItemTemplate><%# Eval("aula.grado.nombre") %></ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField HeaderText="Vacantes disponibles">
                                    <ItemTemplate><%# Eval("vacantes_disponibles") %></ItemTemplate>
                                </asp:TemplateField>
                            </Columns>
                            <RowStyle CssClass="data-row" />
                        </asp:GridView>
